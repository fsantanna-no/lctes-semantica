...

\begin{definition}\label{def.pot}
  The potency of a program~$p$ in reaction to event~$e$,
  denoted~$\pot(p,e)$, is the maximum number of~$\ceu{\EmitInt}$ expressions
  that can be reached during a reaction of~$p$ to~$e$.  Formally,
  $\pot(p,e)$ is defined as follows:
  \[
    \pot(p,e)=\pot'(\bcast(p,e))\,,
  \]
  where
  \begin{enumerate}[(a)]
  \item$\pot'(p)=1$, if~$p=\ceu{\EmitInt(e)}$;
    %%
  \item$\pot'(p)=\max\{\pot'(p_1),\pot'(p_2)\}$,\\
    if~$p=\ceu{\IfElse{\Mem(\Id)}{p_1}{p_2}}$;
    %%
  \item$\pot'(p)=\pot'(p_1)$, if~$p=\ceu{\Loop{p_1}}$
    or~$p=\ceu{\Every{e}\,{p_1}}$;
    %%
  \item$\pot'(p)=\pot'(p_1)+\pot'(p_2)$, if~$p=p_1;\,p_2$
    or~$p=\ceu{p_1\AtLoop{p_2}}$
    with~$p_1\ne\ceu{\Break},\ceu{\AwaitExt(e)}$,
    or~$p=\ceu{{p_1}\And{p_2}}$ or~$p=\ceu{{p_1}\Or{p_2}}$
    or~$p=\ceu{{p_1}\AtAnd{p_2}}$ or~$p=\ceu{{p_1}\AtOr{p_2}}$;
    %%
  \item$\pot'(p)=0$, otherwise.
  \end{enumerate}
  Note that~$\pot'$ only considers the~$\ceu{\EmitInt}$ expressions that
  occur before the first~$\ceu{\AwaitExt}$ in a given execution path.  This
  is because such~$\ceu{\AwaitExt}$ act as barriers that cannot be consumed
  in the reaction to the original event~$e$.
\end{definition}
...
\begin{definition}\label{def.rank}
  ...
  that~$i=\pot(p,e)$, and $j=n$~if~$e=\nil$ or~$j=n+1$ otherwise.
\end{definition}
...
\begin{proof}\strut
  ...
  \item Suppose
    \[
      \<p,n,e>\outpop\<p',n',e'>\,.
    \]
    By~\R{pop}, $p=p'$, $n>0$, $n'=n-1$, and~$e=e'=\nil$.
    %%
...

\begin{lemma}\label{lem.rank-nst}
If~$\delta\nst\delta'$ then~$\rank(\delta)\ge\rank(\delta')$.
\end{lemma}
\begin{proof}
  By induction on the structure of~$\nst$ derivations.
  Let~$\delta=\<p,n,e>$, $\delta'=\<p',n',e'>$, $\rank(\delta)=\<i,j>$,
  and~$\rank(\delta')=\<i',j'>$.  Suppose there is a derivation~$\pi$
  such that
  \[
    \pi\Vdash\<p,n,e>\nst\<p',n',e'>\,.
  \]
  By definition of~$\nst$, we have that~$e=\nil$ and $n=n'$.  Depending on
  the structure of program~$p$, there are~11 possibilities.  In each one of
  them, we show that~$\rank(\delta)\ge\rank(\delta')$.
  ...
  \begin{case}
    ...
    and~$e'=e_1$.
    Thus~${\rank(\delta)={\<1,n>}}>{\<0,n+1>=\rank(\delta')}$.
  \end{case}
  ...
  \begin{case}
    ...
    \begin{subcase}
      ...
      and~$e'=\nil$.  Thus~$\rank(\delta)=\<i,n>$
      and~$\rank(\delta')=\<j,n>$, where~$i=\max\{pot'(p_1),pot'(p_2)\}$
      and~$j=\pot'(p_1)$.  Therefore~$i>j$.
    \end{subcase}
    \begin{subcase}
      $\eval(\ceu{\Mem(\Id)})$~is false.
      %%
  ...
  \begin{case}
    ...
    \begin{subcase}
      $p_1=\ceu{\Nop}$.
      %%
      Then~$\pi$ is an instance of~\R{seq-nop}.  TODO.
    \end{subcase}
    \begin{subcase}
      $p_1=\ceu{\Break}$.
      %%
      Then~$\pi$ is an instance of~\R{seq-brk}.  TODO.
    \end{subcase}
    \begin{subcase}
      \label{lem.rank-nst.seq-adv}
      $p_1\ne\ceu{\Nop},\ceu{\Break}$.
      %%
      Then~$\pi$ is an instance of~\R{seq-adv}.
      Thus there is a derivation~$\pi'$ such that
      \[
        \pi'\Vdash\<p_1,n,\nil>\nst\<p_1',n,e_1'>\,,
      \]
      for some~$p_1'\in\P$ and~$e_1'\in\E$.  Thus~$p'=p_1';p_2$
      and~$e'=e_1'$.  By the induction hypothesis,
      \begin{equation}
      ...
      \begin{subsubcase}
        ...
        \end{align*}
        By~\eqref{lem.rank-nst.seq-adv.eq1} and from the fact
        that~$e_1'=\nil$, $\pot'(p_1)\ge\pot'(p_1')$.  Therefore,
        $\rank(\delta)\ge\rank(\delta')$.
      \end{subsubcase}
      \begin{subsubcase}
        $e'\ne\nil$.
        %%
        Then~$\pi'$ is an instance of~\R{emit-int}, as no other rule affects
        the third component of the output description.
        \[
          \rank(\delta)=\<i,n>
          >\<i-1,n+1>
          =\rank(\delta')\,.
