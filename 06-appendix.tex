...
\begin{definition}\label{def.Hnst}
  ...
  iff~$p=\ceu{\Nop},\ceu{\Break}$ or~$\isblocked(p,n)$~is true.
  Nested-irreducible descriptions serve as normal forms for~$\nst$
  transitions: their program~$p$ and stack-level~$n$ embody the result of an
  exhaustive number of~$\nst$ transitions.  We will write~$\delta_\Hnst$ to
  indicate that description~$\delta$ is nested-irreducible.
\end{definition}
