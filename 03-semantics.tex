...
well-behaved regarding the order of evaluation of composition members.  Its
...
\begin{assumption}[Syntactic restrictions]\strut
  ...
  %%
  \begin{enumerate}[(a)]
  \item\label{ass.x.syn-rest.fin} If~$p=\ceu{\Fin{p_1}}$ then~$p_1$ contain
  ...
  \item\label{ass.x.syn-rest.loop} If~$p=\ceu{\Loop{p_1}}$ then all
    execution paths of~$p_1$ contain at a matching~$\ceu{\Break}$ or
...
(members of set~$P$) and uses Lemma~\ref{lem.props-nst-i} and
...
general.  Therefore, before stating and proving it, we need to characterize
irreducible descriptions in general.
