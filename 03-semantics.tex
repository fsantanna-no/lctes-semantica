...
partial function and that, consequently, the same number of applications
of~$\trans$ will always yield the same result.
...
Lemma~\ref{lem.x.det-out} proceeds by induction on the structure of the
derivation trees produced by applications of the rules for~$\nst$.  Both
lemmas are used in the proof of Theorem~\ref{thm.x.det}, the main result of
this section, which establishes that any given number of applications
of~$\trans$ starting from the same input description will always lead to the
