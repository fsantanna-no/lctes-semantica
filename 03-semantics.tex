...
and encapsulate all side effects and system calls into a single abstract memory
operation.
Memory operations are opaque and behave like in conventional imperative
languages (i.e., reads and writes), having no direct effect on the application
execution flow (e.g., they cannot block or abort execution).
...
We assume that $\ceu{\Mem}$, $\ceu{\AwaitExt}$, and
$\ceu{\AwaitInt}$/$\ceu{\EmitInt}$ expressions do not share identifiers:
...
program, and only applies if the program is blocked (see function~$\isblocked$
