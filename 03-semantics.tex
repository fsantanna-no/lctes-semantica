...
\begin{proof}
  ...
  \begin{case}
    ...
    and~$e'=e_1'$.  There are two subcases.
    \begin{subcase}
      ...
      nor can it consume an~$\ceu{\AwaitExt}$, which means that all
      execution paths in~$p_1'$ still contain a~$\ceu{\Break}$
      or~$\ceu{\AwaitExt}$.  Hence~$\pot'(p')=\pot'(p_1')$.

      There are two further subcases depending on event~$e'$.
      \begin{subsubcase}
        $e'=\nil$.
        %%
...
The idea of the proof is that a series of~$\nst$ or~$\out$ transitions
eventually decrease the rank of the current description until an irreducible
description is reached.  This irreducible description is the final result of
the reaction.
%%
...
\begin{proof}
  ...
  \begin{induction}
    ...
    \begin{case}
      ...
      %%
      Then~$e=\nil$ and, by Theorems~\ref{thm.x.rank-nst-*}
...
maximum number of internal events that~$p$ may emit in a reaction to~$e$.
