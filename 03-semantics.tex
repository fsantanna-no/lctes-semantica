...
The~$\ceu{\Mem(id)}$ primitive represents an assignment or system
...
\R{if-false} are the only to query~$\ceu{\Mem}$ as expressions.
%
...
\begin{definition}
  ...
  or~$\isblocked(p,n)$.\footnote{We often abbreviate ``$p=\ceu{\Nop}$
...
the body of loops, and the restriction mentioned earlier in this section,
regarding the body of~$\ceu{\Fin}$ statements, are formalized in
Assumption~\ref{ass.x.syn-rest} below.  These restrictions are essential to
prove the next theorem, Theorem~\ref{thm.x.term-nst-*}.
