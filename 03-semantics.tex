...
and~$e,e'\in\E\cup\{\nil\}$ are the events emitted before and after
the transition, both possibly the empty event~($\nil$).
...
initialized at stack level $0$ ($n=0$) and with the emitted event
($e=id$).
...
the triple will block with a (possibly) modified program~$p'$, at stack
level~$0$, and with no event emitted~$(\nil)$:
%
\[
  \<p,0,e>\xrightarrow{*}\<p',0,\nil>
\]
