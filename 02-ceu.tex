...
with the LED off.
%
The implementation first declares the \code{BUTTON} and \code{RADIO_RECV} as
input events (ln.~1--2).
...
At first sight, an event iterator for event \code{A} seems like a syntactic
sugar for a \code{loop} followed by and \code{await A}.
...
termination, as we discuss in Section~\ref{sec.sem}:
    they cannot contain \code{break} statements.
