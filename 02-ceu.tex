...
par/or do
    var int* p;
    every p in inc do   // implements subroutine with an event iterator
        *p = *p + 1;
    end
with
    var int v = 1;
    ...
    emit inc(&v);       // calls "inc"
    _assert(v==3);      // asserts result after returns
end
\end{lstlisting}
\vskip-\baselineskip
...
At first sight, event iteration such as ``\code{every e do <...> end}'' seems
...
event).
%
Event iterators have syntactic restrictions and cannot contain \code{await} or
...
contains at least one external \code{await} is extended to contain an
\code{every} statement alternatively.
