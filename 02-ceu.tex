...
|\C{Event handling}|
$\T<id>$ = await $\T<id>$; | \CC{19.5em}{awaits an event and assigns the received value} |
emit $\T<id>$($\T<exp>$); | \CC{19.5em}{emits an event passing a value} |

|\C{Control flow}|
$\T<stmt>$ ; $\T<stmt>$ | \CC{10em}{sequence}     |
if $\T<exp>$ then $\T<stmts>$ else $\T<stmts>$ end | \CC{10em}{conditional}  |
loop do $\T<stmts>$ end | \CC{10em}{repetition}   |
every $\T<id>$ in $\T<id>$ do $\T<stmts>$ end | \CC{10em}{event iteration}   |
finalize [$\T<stmts>$] with $\T<stmts>$ end | \CC{10em}{finalization} |

|\C{Logical parallelism}|
par/or  do $\T<stmts>$ with $\T<stmts>$ end | \CC{13em}{aborts when any side ends}      |
par/and do $\T<stmts>$ with $\T<stmts>$ end | \CC{13em}{terminates when all sides end} |

|\C{Assignment \& Integration with C}|
$\T<id>$ = $\T<exp>$; | \CC{19.5em}{assigns a value to a variable}              |
_$\T<id>$($\T<exps>$) | \CC{19.5em}{calls a C function (id starts with `\_'\,)} |
\end{lstlisting}
\egroup
...
\begin{tikzpicture}[x=.9em,y=.9em,
  ...
  \node[evt](Aevt) at (A) {\textbf{A}};
  \node[evt](Bevt) at (B) {\textbf{B}};
  \node[evt](Cevt) at (C) {\textbf{C}};
  \node[evt](Devt) at (D) {\textbf{D}};
...
At first sight, the constructions ``\code{every e do <...> end}'' and
``\code{loop do await e; <...> end}'' seem to be equivalent.
...
\begin{lstlisting}[numbers=right,basicstyle=\ttfamily\footnotesize]
input void A;
input void B;
var int x = 1;
par/and do
...
\end{lstlisting}
\small{\ax Accesses to \code{x} are never\\\phantom{\ax}\relax concurrent.}
\end{minipage}%
%
\begin{minipage}[h]{0.53\linewidth}
%\begin{lstlisting}
\begin{lstlisting}[xleftmargin=2em,basicstyle=\ttfamily\footnotesize]
input void A;
// (empty line)
var int y = 1;
par/and do
...
\end{lstlisting}
\small{\bx Accesses to \code{y} are concurrent\\
\phantom{\bx}\relax but still deterministic.}
\end{minipage}%
%\rule{8.4cm}{0.37pt}
...
%
%\begin{itemize}
 If \CEU \textbf{passes} a pointer to a system call (ln.~\ax:5), the
          pointer represents a \textbf{local} resource (ln.~\ax:2) that
          requires finalization (ln.~\ax:7).
 If \CEU \textbf{receives} a pointer from a system call return
...
//
\end{lstlisting}
\small{\ax\!Local resource finalization\rlap{.}}
\end{minipage}%
%
...
\end{lstlisting}
\small{\quad\bx\!External resource finalization\rlap{.}}
\end{minipage}%
%\rule{8.4cm}{0.37pt}
\caption{%
